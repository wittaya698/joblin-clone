...
class Command extends BaseCommand {
    ...
    options() {
        return [
            [
                '-n, --limit <num>',
                _('Displays only the first top <num> notes.')
            ],
            [
                '-s, --sort <field>',
                _(
    ...
    async action(args) {
        ...
        queryOptions.uncompletedTodosOnTop = Setting.value(
        ...
        if (options.format && options.format == 'json') {
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                if (options.long) {
                    ...
                    row.push(
                        time.unixMsToLocalDateTime(item.user_updated_time)
                    );
                }
                ...
                if (hasTodos) {
                    if (item.is_todo) {
                        row.push(
                            sprintf('[%s]', !!item.todo_completed ? 'X' : ' ')
                        );
                    } else {
                        row.push('   ');
                    }
