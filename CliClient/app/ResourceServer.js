...
class ResourceServer {
    ...
    async start() {
        ...
        if (!this.port_) {
            this.logger().error(
                'Could not find available port to start resource server. Please report the error at https://github.com/laurent22/joplin'
            );
            return;
        }
        ...
        this.server_.on('request', async (request, response) => {
            ...
            if (resourceId.length < 2) {
                writeResponse(
                    'Error: could not get resource ID from path name: ' +
                        url.pathname
                );
                return;
            }
            ...
            try {
                ...
                if (resource.length > 2)
                    throw new Error(
                        'More than one resource match ID ' + resourceId
                ...
                writeResponse(
                    'Error: could not retrieve resource: ' + error.message
                );
            }
        });
