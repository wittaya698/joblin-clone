...
async function fuzzyMatch(note) {
    let notes = await Note.modelSelectAll(
        'SELECT * FROM notes WHERE is_conflict = 0 AND created_time = ? AND title = ?',
        [note.created_time, note.title]
    );
    ...
    return new Promise((resolve, reject) => {
        ...
        saxStream.on('closetag', function (n) {
            ...
            if (n == 'note') {
                ...
                note.todo_due = dateToTimestamp(
                    noteAttributes['reminder-time'],
                    true
                );
                note.todo_completed = dateToTimestamp(
                    noteAttributes['reminder-done-time'],
                    true
                );
                note.order = dateToTimestamp(
                    noteAttributes['reminder-order'],
                    true
                );
