...
class Application {
    ...
    async loadItem(type, pattern, options = null) {
        ...
        if (output.length > 1) {
            ...
            let msg = _(
                'More than one item match "%s". Please select one:',
                pattern
            );
    ...
    async handleStartFlags_(argv) {
        ...
        while (argv.length) {
            ...
            if (arg == '--ac-line') {
                if (!nextArg)
                    throw new Error(_('Usage: %s', '--ac-line <line>'));
    ...
    async start() {
        ...
        if (process.argv[1].indexOf('joplindev') >= 0) {
            if (!initArgs.profileDir)
                initArgs.profileDir = '/mnt/d/Temp/TestNotes2';
