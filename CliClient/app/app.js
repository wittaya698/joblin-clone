...
class Application {
    ...
    async refreshCurrentFolder() {
        ...
        if (this.currentFolder_)
            newFolder = await Folder.load(this.currentFolder_.id);
    ...
    async loadItems(type, pattern, options = null) {
        if (
            type == BaseModel.TYPE_FOLDER &&
            (pattern == Folder.conflictFolderTitle() ||
                pattern == Folder.conflictFolderId())
        )
            return [Folder.conflictFolder()];
        ...
        if (type == BaseModel.TYPE_NOTE && pattern.indexOf('*') >= 0) {
            ...
            if (type == BaseModel.TYPE_NOTE) {
                if (!parent)
                    throw new Error(_('No notebook has been specified.'));
                item = await ItemClass.loadFolderNoteByField(
                    parent.id,
                    'title',
                    pattern
                );
    ...
    loadCommands_() {
        ...
        fs.readdirSync(__dirname).forEach(path => {
            ...
            for (let i = 0; i < cmd.options().length; i++) {
                ...
                if (options.length == 2)
                    vorpalCmd.option(options[0], options[1]);
                if (options.length == 3)
                    vorpalCmd.option(options[0], options[1], options[2]);
                if (options.length > 3)
                    throw new Error('Invalid number of option arguments');
            }
    ...
    async synchronizer(syncTarget) {
        ...
        this.synchronizers_[syncTarget] = new Synchronizer(
            this.database_,
            fileApi,
    ...
    async start() {
        ...
        this.dbLogger_.addTarget('file', {
            path: profileDir + '/log-database.txt'
        });
        this.dbLogger_.setLevel(initArgs.logLevel);

        this.syncLogger_.addTarget('file', {
            path: profileDir + '/log-sync.txt'
        });
