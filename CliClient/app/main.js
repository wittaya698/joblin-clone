...
commands.push({
    ...
    action: function (args, end) {
        Folder.save({ title: args['notebook'] }, { duplicateCheck: true })
            .then(folder => {
...
commands.push({
    ...
    action: async function (args, end) {
        ...
        try {
            ...
            let tempFilePath =
                Setting.value('profileDir') + '/tmp/' + Note.systemPath(note);
...
commands.push({
    ...
    action: async function (args, end) {
        try {
            ...
            let note = null;
            if (args.note)
                note = await loadItem(BaseModel.TYPE_NOTE, args.note);

            if (args.command == 'remove' && !tag)
                throw new Error(_('Tag does not exist: "%s"', args.tag));

            if (args.command == 'add') {
                if (!note)
                    throw new Error(_('Note does not exist: "%s"', args.note));
    ...
    action: async function (args, end) {
        try {
            ...
            if (options.reverse === true)
                queryOptions.orderByDir =
                    queryOptions.orderByDir == 'ASC' ? 'DESC' : 'ASC';
...
commands.push({
    ...
    action: async function (args, end) {
        if (args.options.stats) {
            ...
            this.log(
                _('Total: %d/%d', report.total.synced, report.total.total)
            );
        } else {
            let options = {
                onProgress: report => {
                    let line = [];
                    if (report.remotesToUpdate)
                        line.push(
                            _(
                                'Items to upload: %d/%d.',
            ...
            this.log(
                _('Synchronization target: %s', Setting.value('sync.target'))
            );
            ...
            if (folderTitle) {
                ...
                if (!folder) {
                    let ok = await cmdPromptConfirm(
                        this,
                        _(
                            'Folder does not exists: "%s". Create it?',
                            folderTitle
                        )
                    );
                    if (!ok) {
            ...
            let options = {
                ...
                onProgress: progressState => {
                    ...
                    if (progressState.updated)
                        line.push(_('Updated: %d.', progressState.updated));
                    if (progressState.skipped)
                        line.push(_('Skipped: %d.', progressState.skipped));
...
function handleStartFlags(argv) {
    argv = argv.slice(0);
...
function escapeShellArg(arg) {
    if (arg.indexOf('"') >= 0 && arg.indexOf("'") >= 0)
        throw new Error(
            _(
                'Command line argument "%s" contains both quotes and double-quotes - aborting.',
                arg
            )
        ); // Hopeless case
    let quote = '"';
