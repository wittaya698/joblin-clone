...
class Command extends BaseCommand {
    ...
    async action(args) {
        ...
        if (folderTitle) {
            ...
            if (!folder)
                throw new Error(_('Notebook not found: "%s"', folderTitle));
        }
        ...
        for (let i = 0; i < notes.length; i++) {
            const note = notes[i];
            const parent = parents[note.parent_id]
                ? parents[note.parent_id]
                : await Folder.load(note.parent_id);
            parents[note.parent_id] = parent;

            const idx = note.body.indexOf(pattern);
            let line = '';
            if (idx >= 0) {
                let fragment = note.body.substr(
                    Math.max(0, idx - fragmentLength / 2),
                    fragmentLength
                );
