...
// Critical: to remove when chalk is available
const chalk = {
    cyan: { bold: text => text },
    blue: { underline: text => text }
};
    ...
    buildUi() {
        ...
        folderList.on('currentItemChange', async event => {
            ...
            if (item === '-') {
                let newIndex =
                    event.currentIndex +
                    (event.previousIndex < event.currentIndex ? +1 : -1);
                let nextItem = folderList.itemAt(newIndex);
                if (!nextItem)
                    nextItem = folderList.itemAt(event.previousIndex);

                if (!nextItem) return; // Normally not possible

                let actionType = 'FOLDERS_SELECT';
                if (nextItem.type_ === BaseModel.TYPE_TAG)
                    actionType = 'TAGS_SELECT';
                if (nextItem.type_ === BaseModel.TYPE_SEARCH)
                    actionType = 'SEARCH_SELECT';
            ...
            action: async () => {
                if (this.widget('folderList').hasFocus) {
                    ...
                    if (item.type_ === BaseModel.TYPE_FOLDER) {
                        ...
                        this.stdout(
                            _('To delete a tag, untag the associated notes.')
                        );
                    } else if (item.type_ === BaseModel.TYPE_SEARCH) {
                        this.store().dispatch({
                            type: 'SEARCH_REMOVE',
    ...
    async setupResourceServer() {
        ...
        this.resourceServer_.setLinkHandler(async (path, response) => {
            ...
            if (link.type === 'resource') {
                ...
                if (!resource)
                    throw new Error('No resource with ID ' + resourceId); // Should be nearly impossible
                if (resource.mime)
                ...
                if (processShortcutKeys) {
                    ...
                    if (typeof cmd.action === 'function') {
                        ...
                        if (cmd.action.type === 'prompt') {
                            let promptOptions = {};
                            if ('cursorPosition' in cmd.action)
                                promptOptions.cursorPosition =
                                    cmd.action.cursorPosition;
