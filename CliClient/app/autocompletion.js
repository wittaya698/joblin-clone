...
async function handleAutocompletion(autocompletion) {
    ...
    if (includeLongs || includeShorts) {
        ...
        for (let i = 0; i < options.length; i++) {
            ...
            if (includeLongs && long && long.indexOf(currentWord) === 0)
                output.push(long);
            if (includeShorts && short && short.indexOf(currentWord) === 0)
                output.push(short);
        }
        return output;
    }
    ...
    if (argName == 'note') {
        ...
        const notes = await Note.previews(app().currentFolder().id, {
            titlePattern: currentWord + '*'
        });
        return notes.map(n => n.title);
    }

    if (argName == 'notebook') {
        const folders = await Folder.search({
            titlePattern: currentWord + '*'
        });
