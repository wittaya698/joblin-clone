...
class Exporter {
    async export(options) {
        ...
        const sourceFolderIds = options.sourceFolderIds
            ? options.sourceFolderIds
            : [];
        const sourceNoteIds = options.sourceNoteIds
            ? options.sourceNoteIds
            : [];
        ...
        const exportItem = async (itemType, itemOrId) => {
            ...
            const item =
                typeof itemOrId === 'object'
                    ? itemOrId
                    : await ItemClass.load(itemOrId);

            if (!item) {
                result.warnings.push(
                    'Cannot find item with type ' +
                        itemType +
                        ' and ID ' +
                        JSON.stringify(itemOrId)
                );
                return;
            }
            ...
            if (itemType == BaseModel.TYPE_RESOURCE) {
                ...
                const destResourcePath =
                    resourceDir + '/' + basename(sourceResourcePath);
        ...
        for (
            ...
            if (!sourceNoteIds.length)
                await exportItem(BaseModel.TYPE_FOLDER, folderId);
