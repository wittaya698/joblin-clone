...
const url =
    'https://api.github.com/repos/wittaya698/joplin-clone/releases/latest';
...
function downloadUrl(release, os) {
    ...
    for (let i = 0; i < release.assets.length; i++) {
        ...
        if (name.indexOf('.dmg') > 0 && os === 'macos')
            return asset.browser_download_url;
        if (name.indexOf('.exe') > 0 && os === 'windows')
            return asset.browser_download_url;
        if (name.indexOf('.AppImage') > 0 && os === 'linux')
            return asset.browser_download_url;
    }
}
...
function readmeContent() {
    if (!fs.existsSync(readmePath))
        throw new Error('Cannot find ' + readmePath);
...
async function main() {
    ...
    if (waitForVersion)
        console.info(
            'Waiting for version ' +
                waitForVersion +
                ' to be released before updating readme...'
        );
    ...
    if (winUrl)
        content = content.replace(
            /(\(https:\/\/github.com\/laurent22\/joplin\/releases\/download\/.*?\.exe\))/,
            '(' + winUrl + ')'
        );
    if (macOsUrl)
        content = content.replace(
            /(\(https:\/\/github.com\/laurent22\/joplin\/releases\/download\/.*?\.dmg\))/,
            '(' + macOsUrl + ')'
        );
    if (linuxUrl)
        content = content.replace(
            /(\(https:\/\/github.com\/laurent22\/joplin\/releases\/download\/.*?\.AppImage\))/,
            '(' + linuxUrl + ')'
        );
