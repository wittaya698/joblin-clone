...
class Synchronizer {
    ...
    syncAction(localItem, remoteItem, deletedLocalPaths) {
        let output = this.syncActions(
            localItem ? [localItem] : [],
            remoteItem ? [remoteItem] : [],
            deletedLocalPaths
        );
        if (output.length !== 1)
            throw new Error('Invalid number of actions returned');
            ...
            if (!remote) {
                ...
                if (this.itemIsOlderThan(remote, local.syncTime)) {
                    ...
                    if (local.isDir) {
                        ...
                        action.solution = [
                            {
                                type: 'copy-to-remote-conflict-dir',
                                dest: 'local'
                            },
                            {
                                type: 'copy-to-local-conflict-dir',
                                dest: 'local'
                            },
    ...
    processState_uploadChanges() {
        ...
        return this.api()
                    ...
                    chain.push(() => {
                        ...
                        if (c.type == Change.TYPE_NOOP) {
                            ...
                            p = this.loadParentAndItem(c).then(result => {
                                ...
                                let remoteFile = this.remoteFileByPath(
                                    remoteFiles,
                                    path
                                );
                                let p = null;
                            ...
                            p = this.loadParentAndItem(c).then(result => {
                                ...
                                if (
                                    remoteFile &&
                                    remoteFile.updatedTime > lastSyncTime
                                ) {
                                    console.info(
                                        'CONFLICT:',
                                        lastSyncTime,
                                ...
                                return p.then(() => {
                                    if (
                                        ...
                                        return this.api().put(
                                            ...
                                            Folder.toFriendlyString(result.item)
                                        );
                                    } else {
                                        return this.api().put(
                                            path,
            ...
            .catch(error => {
                Log.error(
                    'Synchronization was interrupted due to an error:',
                    error
                );
            })
            .then(() => {
    ...
    processState_downloadChanges() {
        ...
        //                 chain.push(() => {
        //                     return ItemClass.delete(syncOp.item_id, {
        //                         trackChanges: false
        //                     });
        //                 });
        //             }
            ...
            if (action.type == 'create') {
                if (action.dest == 'remote') {
                    if (item.isDir) {
                        ...
                        return this.api().put(
                            path,
                            Note.toFriendlyString(item.dbItem)
                        );
                    }
                }
            }
    ...
    processLocalItem(dbItem) {
        //console.info(dbItem);
        let localItem = null;
        return this.dbItemToSyncItem(dbItem)
            .then(r => {
                localItem = r;
    ...
    start() {
        ...
        if (this.state() != 'idle') {
            ...
            return Promise.reject(
                'Cannot start synchronizer because synchronization already in progress. State: ' +
                    this.state()
            );
        }
        ...
        return this.api()
            ...
            .then(items => {
                ...
                let readItems = () => {
                    isReading = true;
                    return NoteFolderService.itemsThatNeedSync(context, limit)
                        .then(result => {
