...
class ItemList extends React.Component {
    ...
    itemCount(props = null) {
        ...
        return this.props.items
            ...
            : this.props.itemComponents.length;
    }
    ...
    updateStateItemIndexes(props = null, height = null) {
        ...
        const topItemIndex = Math.max(
            0,
            Math.floor(this.scrollTop_ / props.itemHeight)
        );
        const visibleItemCount = Math.ceil(height / props.itemHeight);

        let bottomItemIndex = topItemIndex + visibleItemCount - 1;
        if (bottomItemIndex >= this.itemCount(props))
            bottomItemIndex = this.itemCount(props) - 1;
    ...
    render() {
        ...
        if (this.props.items) {
            ...
            itemComps = [
                blankItem(
                    'top',
                    this.state.topItemIndex * this.props.itemHeight
                )
            ];
