...
function changeSelectedNotes(state, action) {
    const noteIds =
        'id' in action ? (action.id ? [action.id] : []) : action.ids;
    let newState = Object.assign({}, state);
    ...
    if (action.type === 'NOTE_SELECT_ADD') {
        ...
        newState.selectedNoteIds = ArrayUtils.unique(
            newState.selectedNoteIds.concat(noteIds)
        );
        return newState;
    }
    ...
    if (action.type === 'NOTE_SELECT_TOGGLE') {
        ...
        if (newState.selectedNoteIds.indexOf(noteIds[0]) >= 0) {
            newState = changeSelectedNotes(state, {
        ...
        switch (action.type) {
            ...
            case 'NOTE_SELECT_EXTEND':
                ...
                if (!newState.selectedNoteIds.length) {
                    ...
                    const selectRangeId1 =
                        state.selectedNoteIds[state.selectedNoteIds.length - 1];
                    ...
                    for (let i = 0; i < state.notes.length; i++) {
                        ...
                        if (
                            !selectionStarted &&
                            (id === selectRangeId1 || id === selectRangeId2)
                        ) {
                        ...
                        if (
                            selectionStarted &&
                            newSelectedNoteIds.indexOf(id) < 0
                        ) {
                            newSelectedNoteIds.push(id);
                        }
                    }
            ...
            case 'FOLDER_SELECT':
                ...
                if (!action.id) {
                    newState.notesParentType = defaultNotesParentType(
                        state,
                        'Folder'
                    );
                } else {
            ...
            case 'NOTE_UPDATE_ONE':
                ...
                const noteIsInFolder = function (note, folderId) {
                    if (note.is_conflict)
                        return folderId === Folder.conflictFolderId();
