    ...
    async start(argv) {
        ...
        if (process.argv[1].indexOf('joplindev') >= 0) {
            if (!initArgs.profileDir)
                initArgs.profileDir = '/Users/macbookair/Temp/TestNotes2';
            initArgs.logLevel = Logger.LEVEL_DEBUG;
            initArgs.env = 'dev';
        }

        let appName = initArgs.env == 'dev' ? 'joplindev' : 'joplin';
        if (Setting.value('appId').indexOf('-desktop') >= 0)
            appName += '-desktop';
        ...
        const extraFlags = await this.readFlagsFromFile(
            profileDir + '/flags.txt'
        );
