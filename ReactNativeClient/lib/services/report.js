...
class ReportService {
    ...
    async basicItemList(option = null) {
        ...
        for (let i = 0; i < itemTypes.length; i++) {
            ...
            const items = await ItemClass.modelSelectAll(
                'SELECT items.id, items.updated_time, sync_items.sync_time FROM ' +
                    ItemClass.tableName() +
                    ' items JOIN sync_items ON sync_items.item_id = items.id'
            );
    ...
    async status(syncTarget) {
        ...
        let sections = [];
        let section = {
            title: _('Sync status (synced items / total items)'),
            body: []
        };
        ...
        for (let n in r.items) {
            ...
            section.body.push(
                _('%s: %d/%d', n, r.items[n].synced, r.items[n].total)
            );
        }
        ...
        for (let i = 0; i < alarms.length; i++) {
            ...
            section.body.push(
                _(
                    'On %s: %s',
                    time.formatMsToLocal(alarm.trigger_time),
                    note.title
                )
            );
        }
