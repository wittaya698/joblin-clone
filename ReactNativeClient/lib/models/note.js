...
class Note extends BaseItem {
    ...
    static geolocationUrl(note) {
        ...
        return sprintf(
            'https://www.openstreetmap.org/?lat=%s&lon=%s&zoom=20',
            note.latitude,
            note.longitude
        );
    }
    ...
    static async previews(parentId, options = null) {
        ...
        if (options.uncompletedTodosOnTop && hasTodos) {
            ...
            if (hasNotes && hasTodos) {
                cond.push(
                    '(is_todo = 0 OR (is_todo = 1 AND todo_completed > 0))'
                );
            } else {
    ...
    static async updateGeolocation(noteId) {
        ...
        while (true) {
            ...
            if (startWait + 1000 * 20 < time.unixMs()) {
                this.logger().warn(
                    'Failed to update geolocation for: timeout: ' + noteId
                );
                return;
            }
        }
        ...
        let geoData = null;
        if (
            this.geolocationCache_ &&
            this.geolocationCache_.timestamp + 1000 * 60 * 10 > time.unixMs()
        ) {
    ...
    static filter(note) {
        ...
        if ('longitude' in output)
            output.longitude = Number(
                !output.longitude ? 0 : output.longitude
            ).toFixed(8);
        if ('latitude' in output)
            output.latitude = Number(
                !output.latitude ? 0 : output.latitude
            ).toFixed(8);
        if ('altitude' in output)
            output.altitude = Number(
    ...
    static dueNotes() {
        return this.modelSelectAll(
            'SELECT id, title, body, is_todo, todo_due, todo_completed, is_conflict FROM notes WHERE is_conflict = 0 AND is_todo = 1 AND todo_completed = 0 AND todo_due > ?',
            [time.unixMs()]
        );
    }
