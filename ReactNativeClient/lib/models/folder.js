...
class Folder extends BaseItem {
    ...
    static async noteCount(parentId) {
        let r = await this.db().selectOne(
            'SELECT count(*) as total FROM notes WHERE is_conflict = 0 AND parent_id = ?',
            [parentId]
        );
        return r ? r.total : 0;
    }
    ...
    static defaultFolder() {
        return this.modelSelectOne(
            'SELECT * FROM folders ORDER BY created_time DESC LIMIT 1'
        );
    }
    ...
    static async save(o, options = null) {
        if (options && options.duplicateCheck === true && o.title) {
            ...
            if (existingFolder)
                throw new Error(
                    _(
                        'A notebook with this title already exists: "%s"',
                        o.title
                    )
                );
        }
