...
class BaseItem extends BaseModel {
    ...
    static async syncedItemIds(syncTarget) {
        ...
        let temp = await this.db().selectAll(
            'SELECT item_id FROM sync_items WHERE sync_time > 0 AND sync_target = ?',
            [syncTarget]
        );
        let output = [];
    ...
    static async batchDelete(ids, options = null) {
        ...
        if (this.modelType() == BaseModel.TYPE_NOTE) {
            const conflictNotes = await this.db().selectAll(
    ...
    static deletedItems(syncTarget) {
        return this.db().selectAll(
            'SELECT * FROM deleted_items WHERE sync_target = ?',
            [syncTarget]
        );
    }
    ...
    static async deletedItemCount(syncTarget) {
        let r = await this.db().selectOne(
            'SELECT count(*) as total FROM deleted_items WHERE sync_target = ?',
            [syncTarget]
        );
        return r['total'];
    }
    ...
    static remoteDeletedItem(syncTarget, itemId) {
        return this.db().exec(
            'DELETE FROM deleted_items WHERE item_id = ? AND sync_target = ?',
            [itemId, syncTarget]
        );
    }
    ...
    static updateSyncTimeQueries(syncTarget, item, syncTime) {
        ...
        if (!itemType || !itemId || syncTime === undefined)
            throw new Error('Invalid parameters in updateSyncTimeQueries()');
