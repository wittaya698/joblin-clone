...
class Database {
        ...
        if (this.inTransaction_) {
            return new Promise((resolve, reject) => {
                let iid = setInterval(() => {
                    if (!this.inTransaction_) {
                        clearInterval(iid);
                        this.transactionExecBatch(queries)
                            .then(() => {
                                resolve();
                            })
                            .catch(error => {
    ...
    refreshTableFields() {
        ...
        return this.selectAll(
            ...
            for (let i = 0; i < tableRows.length; i++) {
                ...
                if (tableName == 'ios_metadata') continue;
                    ...
                    return this.selectAll(
                        ...
                        for (let i = 0; i < pragmas.length; i++) {
                            ...
                            if (
                                ...
                                defaultValue = defaultValue.substr(
                                    1,
                                    defaultValue.length - 2
                                );
                            }
    ...
    initialize() {
        ...
        return this.selectOne('SELECT * FROM version LIMIT 1')
            .then(row => {
            ...
            .then(() => {
                ...
                return this.selectAll('SELECT * FROM table_fields').then(
                    rows => {
                ...
                // .then(() => {
                //     let p = this.exec('DELETE FROM notes')
                //         .then(() => {
                ...
                //         .then(() => {
                //             return this.exec(
                //                 'DELETE FROM settings WHERE `key` = "sync.lastRevId"'
                //             );
                //         });
                ...
                //         .then(() => {
                //             return this.exec(
                //                 'DELETE FROM settings WHERE `key` != "clientId"'
                //             );
                //         });
                // });
            })
            .catch(error => {
                ...
                queries.push(
                    this.wrapQuery(
                        'INSERT INTO folders (`id`, `title`, `created_time`) VALUES ("' +
                            uuid.create() +
                            '", "' +
                            _('Default list') +
                            '", ' +
