...
class ItemSyncTime extends BaseModel {
    static time(itemId) {
        ...
        return this.db()
            .selectOne('SELECT * FROM item_sync_times WHERE item_id = ?', [
                itemId
            ])
            .then(row => {
    ...
    static setTime(itemId, time) {
        return this.db()
            ...
            .then(row => {
                ...
                if (row) {
                    p = this.db().exec(
                        'UPDATE item_sync_times SET `time` = ? WHERE item_id = ?',
                        [time, itemId]
                    );
                } else {
                    p = this.db().exec(
                        'INSERT INTO item_sync_times (item_id, `time`) VALUES (?, ?)',
                        [itemId, time]
                    );
                }
    ...
    static deleteTime(itemId) {
        return this.db()
            .exec('DELETE FROM item_sync_times WHERE item_id = ?', [itemId])
            .then(() => {
