...
class Folder extends BaseItem {
    ...
    static serialize(folder) {
        return super.serialize(folder, 'folder', [
            'id',
            'created_time',
            'updated_time',
            'type_'
    ...
    static async defaultFolder() {
        return this.modelSelectOne(
            'SELECT * FROM folders WHERE is_default = 1'
        );
    }
    ...
    static save(o, options = null) {
        return Folder.loadByField('title', o.title).then(existingFolder => {
            if (existingFolder && existingFolder.id != o.id)
                throw new Error(
                    _('A folder with title "%s" already exists', o.title)
                );
            ...
            if ('is_default' in o) {
                if (!o.is_default) {
                    ...
                    delete o.is_default;
                    Log.warn(
                        'is_default property cannot be set to 0 directly. Instead, set the folder that should become the default to 1.'
                    );
                } else {
                    if (!options) options = {};
                    if (!options.transactionNextQueries)
                        options.transactionNextQueries = [];
                    options.transactionNextQueries.push({
                        sql: 'UPDATE folders SET is_default = 0 WHERE id != ?',
                        params: [o.id]
                    });
                }
            }
