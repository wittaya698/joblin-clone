...
class BaseItem extends BaseModel {
    ...
    static getClass(name) {
        ...
        this.classes_[name] = require('lib/models/' +
            name.toLowerCase() +
            '.js')[name];
    ...
    static itemClass(item) {
        ...
        if (typeof item === 'object') {
            ...
            if (Number(item) === BaseModel.MODEL_TYPE_NOTE)
                return this.getClass('Note');
            if (Number(item) === BaseModel.MODEL_TYPE_FOLDER)
                return this.getClass('Folder');
            if (Number(item) === BaseModel.MODEL_TYPE_RESOURCE)
                return this.getClass('Resource');
            if (Number(item) === BaseModel.MODEL_TYPE_TAG)
                return this.getClass('Tag');
    ...
    static async syncedItems() {
        let folders = await this.getClass('Folder').modelSelectAll(
            'SELECT id FROM folders WHERE sync_time > 0'
        );
        let notes = await this.getClass('Note').modelSelectAll(
            'SELECT id FROM notes WHERE is_conflict = 0 AND sync_time > 0'
        );
        let resources = await this.getClass('Resource').modelSelectAll(
            'SELECT id FROM resources WHERE sync_time > 0'
        );
        let tags = await this.getClass('Tag').modelSelectAll(
            'SELECT id FROM tags WHERE sync_time > 0'
        );
    ...
    static unserialize_format(type, propName, propValue) {
        ...
        if (['created_time', 'updated_time'].indexOf(propName) >= 0) {
            ...
            propValue = Database.formatValue(
                ItemClass.fieldType(propName),
                propValue
            );
        }
    ...
    static async unserialize(content) {
        ...
        for (let i = lines.length - 1; i >= 0; i--) {
            ...
            if (state == 'readingProps') {
                ...
                if (p < 0)
                    throw new Error(
                        'Invalid property format: ' + line + ': ' + content
                    );
        ...
        if (body.length < 3)
            throw new Error(
                'Invalid body size: ' + body.length + ': ' + content
            );
        ...
        if (output.type_ == BaseModel.MODEL_TYPE_NOTE)
            output.body = body.join('\n');

        for (let n in output) {
            if (!output.hasOwnProperty(n)) continue;
            output[n] = await this.unserialize_format(
                output.type_,
                n,
                output[n]
            );
