...
class BaseItem extends BaseModel {
    ...
    static itemClass(item) {
        ...
        if (typeof item === 'object') {
            ...
            // if (Number(item) === BaseModel.TYPE_NOTE)
            //     return this.getClass('Note');
            // if (Number(item) === BaseModel.TYPE_FOLDER)
            //     return this.getClass('Folder');
            // if (Number(item) === BaseModel.TYPE_RESOURCE)
            //     return this.getClass('Resource');
            // if (Number(item) === BaseModel.TYPE_TAG)
    ...
    static async syncedItems() {
        ...
        let noteTags = await this.getClass('NoteTag').modelSelectAll(
            'SELECT id FROM note_tags WHERE sync_time > 0'
        );
        return folders
            .concat(notes)
            .concat(resources)
