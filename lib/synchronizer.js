            ...
            while (true) {
                ...
                for (let i = 0; i < locals.length; i++) {
                    ...
                    if (donePaths.indexOf(path) > 0)
                        throw new Error(
                            sprintf(
                                'Processing a path that has already been done: %s. sync_time was not updated?',
                                path
                            )
                        );
                    ...
                    if (!remote) {
                        if (!local.sync_time) {
                            ...
                            action =
                                local.type_ == BaseModel.MODEL_TYPE_NOTE
                                    ? 'noteConflict'
                                    : 'itemConflict';
                            reason =
                                'remote has been deleted, but local has changes';
                        }
                    } else {
                    ...
                    if (
                        local.type_ == BaseModel.MODEL_TYPE_RESOURCE &&
                        (action == 'createRemote' ||
            ...
            while (true) {
                ...
                for (let i = 0; i < remotes.length; i++) {
                    ...
                    if (action == 'createLocal' || action == 'updateLocal') {
                        ...
                        if (
                            newContent.type_ == BaseModel.MODEL_TYPE_RESOURCE &&
                            action == 'createLocal'
                        ) {
                        ...
                        if (newContent.type_ == BaseModel.MODEL_TYPE_TAG) {
                            ...
                            await ItemClass.setAssociatedNotes(
                                newContent.id,
                                noteIds
                            );
                        }
